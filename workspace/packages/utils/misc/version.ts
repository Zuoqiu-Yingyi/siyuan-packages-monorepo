...
export function isSemanticVersion(version: string): boolean {
    return /^(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)(?:-(?:0|[1-9]\d*|\d*[a-z-][0-9a-z-]*)(?:\.(?:0|[1-9]\d*|\d*[a-z-][0-9a-z-]*))*)?(?:\+[0-9a-z-]+(?:\.[0-9a-z-]+)*)?$/i.test(version);
}
...
export function versionStringToNumber(version: string): number {
    const [major, minor, patch] = version.split(/[+-]/)[0]!.split(".").map((v) => Number.parseInt(v));
    return (major ?? 0) * 1_0000_0000
        + (minor ?? 0) * 1_0000
        + (patch ?? 0);
}
